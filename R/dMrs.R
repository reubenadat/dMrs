...
pexpweibull = function(q,lambda,alpha,kappa,log.p = FALSE){
	...
	if( any(log_cdf_weib == 0) ){
		# precision fix
		idx = log_cdf_weib == 0
		log_cdf_weib[idx] = -1 * pweibull(q[idx],
			scale = lambda,shape = alpha,lower.tail = FALSE)
	}
...
dexpweibull = function(x,lambda,alpha,kappa,log = FALSE){
	...
	if( any(log_cdf_weib == 0) ){
		# precision fix
		idx = log_cdf_weib == 0
		log_cdf_weib[idx] = -1 * pweibull(q = x[idx],
			scale = lambda,shape = alpha,lower.tail = FALSE)
	}
...
#'	and \code{sex} corresponding to age, observed time, 
